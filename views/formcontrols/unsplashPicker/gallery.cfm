...
<cfoutput>
	<div id="gallery">
		<div v-cloak>
			<input v-model.lazy.trim="keyword" type="text" id="keyword" name="keyword" value="" placeholder="Search for an image..." class="form-control">
				...
				<div class="grid max-w-full mx-auto py-6">
					<div v-for="( photo, key, index ) in photos" :key="photo.id" class="relative">
						...
						<img
							...
							v-on:click.prevent="selectPhoto"
							v-bind:class="[selectedPhotoId === photo.id ? '' : 'opacity-50' ]"
							class="object-cover h-48 w-full rounded-md cursor-pointer bg-gray-100 hover:opacity-100">
